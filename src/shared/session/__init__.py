...
def get_session(params: Optional[dict] = None) -> Optional[Dict[str, Any]]:
    ...
    if session:
        if not _is_session_expired(session):
            ...
            log_debug(
                f"Loaded session in {t1-t0:.2f}s (session found, expired)", verbose
            )
    else:
    ...
    try:
        ...
        # Create session
        session = {"jwt_token": id_token, "aws_credentials": aws_creds}
...
def _save_session(session: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if (
            "aws_credentials" in session_copy
            and "Expiration" in session_copy["aws_credentials"]
        ):
...
def _is_session_expired(session: Dict[str, Any]) -> bool:
    ...
    try:
        expires = datetime.fromisoformat(
            session["aws_credentials"]["Expiration"].replace("Z", "+00:00")
        )
...
def _get_cognito_config() -> Dict[str, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Timeout after {API_TIMEOUT} seconds while fetching Cognito config from {well_known_url}"
        )
                    ...
                    try:
                        if not new_password:
                            new_password = getpass.getpass(
                                "New password required. Enter new password: "
                            )
                        ...
                        if attempt < 2:  # Don't print error on last attempt
                            ...
                            raise RuntimeError(
                                f"Failed to set new password after 3 attempts: {e}"
                            )
                ...
                for attempt in range(3):
                    try:
                        ...
                        if attempt < 2:  # Don't print error on last attempt
                            ...
                            raise RuntimeError(
                                f"Failed MFA verification after 3 attempts: {e}"
                            )
            else:
                raise RuntimeError(
                    f"Unsupported Cognito challenge: {resp['ChallengeName']}"
                )
...
def _get_aws_credentials(config: Dict[str, str], id_token: str) -> Dict[str, str]:
    ...
    cognito_identity = boto3.client(
        "cognito-identity", region_name=config.get("Region", "us-west-2")
    )
    ...
    try:
        resp = cognito_identity.get_id(
            ...
            Logins={
                f'cognito-idp.{config["Region"]}.amazonaws.com/{config["UserPoolId"]}': id_token
            },
        )
...
if __name__ == "__main__":
    ...
    params = {
        ...
        if k in ("username", "password", "new_password", "verbose") and v is not None
    }
    try:
        session = get_session(params)
        if session:
            print(
                "Current session valid until:", session["aws_credentials"]["Expiration"]
            )
            exit(0)
        else:
